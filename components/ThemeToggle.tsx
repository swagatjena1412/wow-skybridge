...
export function ThemeToggle() {
  ...
  // next-themes sets the theme after hydration. Track first-interaction
  // with local state instead of useEffect to avoid the setState-in-effect lint.
  const [interacted, setInteracted] = useState(false);
  const current = interacted ? (theme ?? "system") : "system";

  const handleClick = useCallback(() => {
    setInteracted(true);
    setTheme(NEXT[theme ?? "system"] ?? "light");
  }, [theme, setTheme]);
